...
def select_series_title_with_most_human_characters
  "SELECT series.title from series JOIN books ON series.id = books.id JOIN character_books ON character_books.book_id = books.id JOIN characters ON character_books.character_id = characters.id WHERE characters.species = 'human' GROUP BY series.title ORDER BY COUNT(*)"
end
